...
export function CommunityRightPanelAdmin({ community }: { community: Community }) {
  return (
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          {community.adminStats.map((stat) => (
            <div key={stat.label} className="rounded-lg border border-border p-2.5">
              <p className="text-lg leading-tight font-bold text-ink">{stat.value}</p>
          ...
          <div className="flex flex-col gap-2.5">
            {community.recentActivity.map((item, i) => (
              <p key={i} className="text-sm leading-snug text-ink-muted">
                ...
                {item.action}
                <span className="block text-xs text-ink-faint">{item.timeLabel}</span>
              </p>
            ))}
          </div>
        </div>
