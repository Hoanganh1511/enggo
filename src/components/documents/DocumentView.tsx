...
import { getDocumentExtensions, DOC_PROSE_CLASS } from "./document-extensions";

type TocItem = { id: string; text: string; level: number };

// Trang doc tai lieu (read-only) - render Tiptap JSON giong het editor (cung
// bo extension) + muc luc TU DONG sinh tu cac heading. Chu so huu thay them
// nut Sua / Ghim / Xoa; ai cung thay nut Chia se (copy link).
export function DocumentView({ document: doc }: { document: ApiDocument }) {
  ...
  const editor = useEditor({
    extensions: getDocumentExtensions(),
    content: doc.content,
    editable: false,
    immediatelyRender: false,
    editorProps: { attributes: { class: DOC_PROSE_CLASS } },
  });
  ...
  function remove() {
    ...
    startTransition(async () => {
      ...
      router.push(`/u/${doc.author.username}/docs`);
    });
  }
            ...
            <div className="ml-auto flex items-center gap-1.5">
              ...
              {doc.isOwner && (
                <>
                  ...
                  <Link
                    href={`/u/${doc.author.username}/docs/${doc.slug}/edit`}
                    className="flex h-8 cursor-pointer items-center gap-1.5 rounded-full border border-border px-3 text-xs font-semibold text-ink-muted hover:bg-hover-bg"
                  >
